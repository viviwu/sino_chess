...
-(void)setCenterScaningFrame{
    ...
    self.navigationItem.leftBarButtonItem = scanButtonItem;
     ...
     move = 1.0;
    //扫描区域宽、高大小
    float QRWIDTH = 200/320.0*kScreenW;
    
    //创建扫描区域框
    _qrView=[[UIImageView alloc]init];
    _qrView.bounds = CGRectMake(0, 0, QRWIDTH, QRWIDTH);
    _qrView.center = CGPointMake(kScreenW/2.0, (kScreenH-200)/2.0);
    ...
    qrBacView.frame = CGRectMake(0, 0, kScreenW, _qrView.frame.origin.y);
    qrBacView.backgroundColor = [UIColor blackColor];
    qrBacView.alpha = 0.1;
    [self.view addSubview:qrBacView];
    
    qrBacView = [[UIView alloc]init];//左
    qrBacView.frame = CGRectMake(0, _qrView.frame.origin.y, _qrView.frame.origin.x,kScreenH - _qrView.frame.origin.y);
    qrBacView.backgroundColor = [UIColor blackColor];
    qrBacView.alpha = 0.1;
    [self.view addSubview:qrBacView];
    
    qrBacView = [[UIView alloc]init];//下
    qrBacView.frame = CGRectMake(_qrView.frame.origin.x, _qrView.frame.origin.y + QRWIDTH, kScreenW - _qrView.frame.origin.x, kScreenH - _qrView.frame.origin.y - QRWIDTH);
    qrBacView.backgroundColor = [UIColor blackColor];
    qrBacView.alpha = 0.1;
    [self.view addSubview:qrBacView];
    
    qrBacView = [[UIView alloc]init];//右
    qrBacView.frame = CGRectMake(_qrView.frame.origin.x + QRWIDTH, _qrView.frame.origin.y, kScreenW - _qrView.frame.origin.x - QRWIDTH, QRWIDTH);
    qrBacView.backgroundColor = [UIColor blackColor];
    qrBacView.alpha = 0.1;
    [self.view addSubview:qrBacView];
    //判断是否可以使用相机
...
- (void)createQRView{
    ...
    //设置扫描有效区域(上、左、下、右)
    [_output setRectOfInterest : CGRectMake (( _qrView.frame.origin.y )/ kScreenH ,(_qrView.frame.origin.x)/ kScreenW , QRWIDTH / kScreenH , QRWIDTH / kScreenW )];
    // Preview
