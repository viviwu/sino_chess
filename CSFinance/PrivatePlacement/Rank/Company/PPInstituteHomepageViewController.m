//
//  PPInstituteHomepageViewController.m
//  SwipeTableView
//
...
#import "PPInstituteHomepageViewController.h"

#import <objc/message.h>
#import "SwipeTableView.h"
#import "STRefresh.h"
#import "XWSegmentControl.h"
...
@interface PPInstituteHomepageViewController ()<SwipeTableViewDataSource,SwipeTableViewDelegate,UIGestureRecognizerDelegate,UIViewControllerTransitioningDelegate>

@property (nonatomic, strong) SwipeTableView * swipeTableView;
@property (nonatomic, strong) STHeaderView * tableViewHeader;
@property (nonatomic, strong) XWSegmentControl * segmentBar;
@property (nonatomic, strong) XWTableView * tableView;
//@property (nonatomic, strong) CustomCollectionView * collectionView;
@property (nonatomic, strong) XWCollectionView * collectionView;
@property (nonatomic, strong) NSMutableDictionary * dataDic;

@end

@implementation PPInstituteHomepageViewController
...
- (void)viewDidLoad {
    ...
    // nav bar
    UIBarButtonItem * rightBarItem = [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemSearch target:self action:nil];
...
- (UIView *)tableViewHeader {
    if (nil == _tableViewHeader) {
        UIImage * headerImage = [UIImage imageNamed:@"goldengate"];
        // swipe header
        self.tableViewHeader = [[STHeaderView alloc]init];
        _tableViewHeader.frame = CGRectMake(0, 0, kScreenW, 128.0);
...
- (XWSegmentControl * )segmentBar {
    if (nil == _segmentBar) {
        self.segmentBar = [[XWSegmentControl alloc]initWithItems:@[@"公司概况",@"旗下产品",@"基金经理",@"官方动态"]];
        _segmentBar.st_size = CGSizeMake(kScreenW, 40);
...
- (XWTableView *)tableView {
    if (nil == _tableView) {
        _tableView = [[XWTableView alloc]initWithFrame:_swipeTableView.bounds style:UITableViewStylePlain];
...
- (XWCollectionView *)collectionView{
    if (nil==_collectionView) {
        _collectionView = [[XWCollectionView alloc]initWithFrame:_swipeTableView.bounds];
//        _collectionView.backgroundColor = RGBColor(255, 255, 225);
    }
    return _collectionView;
}
...
- (void)getDataAtIndex:(NSInteger)index {
    ...
    switch (index) {
        case 0:
            ...
            break;
        case 1:
            numberOfRows = (_type == STControllerTypeNormal)?20:18;
            break;
        case 2:
...
- (NSArray<XWCellLayout*>*)sectionLayouts{
    ...
    for (int k=0; k<3; k++) {
        ...
        if(k==0){
            ...
            for (int i=0; i<6; i++) {
                XWCellLayout * cellLayout = [[XWCellLayout alloc]init];
                cellLayout.size = CGSizeMake(kScreenW/2-7.5, kScreenW/4);
                [celllayouts addObject:cellLayout];
            }
        }else{
            for (int i=0; i<5; i++) {
                XWCellLayout * cellLayout = [[XWCellLayout alloc]init];
                cellLayout.size = CGSizeMake(kScreenW-10.0, kScreenW/6);
                [celllayouts addObject:cellLayout];
            }
        }
...
- (UIScrollView *)swipeTableView:(SwipeTableView *)swipeView viewForItemAtIndex:(NSInteger)index reusingView:(UIScrollView *)view
{
    switch (_type) {
        ...
        {
            XWTableView * tableView = (XWTableView *)view;
            // 重用
            if (nil == tableView) {
                tableView = [[XWTableView alloc]initWithFrame:swipeView.bounds style:UITableViewStylePlain];
        ...
        case STControllerTypeHiddenNavBar:
        {
            ...
            if (index == 2 || index == 3) {
                ...
                XWTableView * tableView = self.tableView;
                // 获取当前index下item的数据，进行数据刷新
                id data = _dataDic[@(index)];
                [tableView refreshWithData:data atIndex:index];
