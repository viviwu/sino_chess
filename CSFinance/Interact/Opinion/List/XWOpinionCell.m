...
@implementation XWOpinionTitleView
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _titleLabel.size = CGSizeMake(kScreenW - 100, self.height);
...
@implementation XWOpinionProfileView {
    BOOL _trackingTouch;
}
- (instancetype)initWithFrame:(CGRect)frame {
    if (frame.size.width == 0 && frame.size.height == 0) {
...
- (void)setVerifyType:(WBUserVerifyType)verifyType {
    ...
    switch (verifyType) {
        case WBUserVerifyTypeStandard: {
            ...
            _avatarBadgeView.image = [XWOpinionHelper imageNamed:@"avatar_vip"];
        } break;
        case WBUserVerifyTypeClub: {
            _avatarBadgeView.hidden = NO;
            _avatarBadgeView.image = [XWOpinionHelper imageNamed:@"avatar_grassroot"];
        } break;
        default: {
...
@implementation XWOpinionCardView {
    BOOL _isRetweet;
}
...
- (void)setWithLayout:(XWOpinionLayout *)layout isRetweet:(BOOL)isRetweet {
    ...
    switch (isRetweet ? layout.retweetCardType : layout.cardType) {
        case XWOpinionCardTypeNone: {
            
        } break;
        case XWOpinionCardTypeNormal: {
        ...
        case XWOpinionCardTypeVideo: {
            ...
            [_button setImage:[XWOpinionHelper imageNamed:@"multimedia_videocard_play"] forState:UIControlStateNormal];
...
@implementation XWOpinionToolbarView
- (instancetype)initWithFrame:(CGRect)frame {
    if (frame.size.width == 0 && frame.size.height == 0) {
        frame.size.width = kScreenW;
    ...
    _repostImageView = [[UIImageView alloc] initWithImage:[XWOpinionHelper imageNamed:@"timeline_icon_retweet"]];
    _repostImageView.centerY = self.height / 2;
    [_repostButton addSubview:_repostImageView];
    _commentImageView = [[UIImageView alloc] initWithImage:[XWOpinionHelper imageNamed:@"timeline_icon_comment"]];
    _commentImageView.centerY = self.height / 2;
    [_commentButton addSubview:_commentImageView];
    _likeImageView = [[UIImageView alloc] initWithImage:[XWOpinionHelper imageNamed:@"timeline_icon_unlike"]];
    ...
    [_repostButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id sender) {
        XWOpinionCell *cell = weak_self.cell;
...
- (UIImage *)likeImage {
    ...
    dispatch_once(&onceToken, ^{
        img = [XWOpinionHelper imageNamed:@"timeline_icon_like"];
    });
    return img;
}
...
- (UIImage *)unlikeImage {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        img = [XWOpinionHelper imageNamed:@"timeline_icon_unlike"];
    });
    return img;
}
...
- (void)setLiked:(BOOL)liked withAnimation:(BOOL)animation {
    XWOpinionLayout *layout = _cell.statusView.layout;
    ...
    NSString *newCountDesc = newCount > 0 ? [XWOpinionHelper shortedNumberDesc:newCount] : @"赞";
...
@implementation XWOpinionTagView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _imageView.image = [XWOpinionHelper imageNamed:@"timeline_icon_locate"];
    _imageView.hidden = YES;
    [self addSubview:_imageView];
...
- (void)setWithLayout:(XWOpinionLayout *)layout {
    if (layout.tagType == XWOpinionTagTypePlace) {
        _label.height = kWBCellTagPlaceHeight;
        _imageView.hidden = NO;
        _button.hidden = NO;
    ...
    } else if (layout.tagType == XWOpinionTagTypeNormal) {
        _imageView.hidden = YES;
        _button.hidden = YES;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _titleView = [XWOpinionTitleView new];
    _titleView.hidden = YES;
    [_contentView addSubview:_titleView];
    
    _profileView = [XWOpinionProfileView new];
    [_contentView addSubview:_profileView];
    
    _vipBackgroundView = [UIImageView new];
    _vipBackgroundView.size = CGSizeMake(kScreenW, 14.0);
    ...
    [_menuButton setImage:[XWOpinionHelper imageNamed:@"timeline_icon_more"] forState:UIControlStateNormal];
    [_menuButton setImage:[XWOpinionHelper imageNamed:@"timeline_icon_more_highlighted"] forState:UIControlStateHighlighted];
    ...
    _retweetBackgroundView.width = kScreenW;
    ...
    _cardView = [XWOpinionCardView new];
    _cardView.hidden = YES;
    [_contentView addSubview:_cardView];
    
    _tagView = [XWOpinionTagView new];
    _tagView.left = kWBCellPadding;
    _tagView.hidden = YES;
    [_contentView addSubview:_tagView];
    
    _toolbarView = [XWOpinionToolbarView new];
...
- (void)setLayout:(XWOpinionLayout *)layout {
    ...
    [_profileView.avatarView setImageWithURL:layout.status.user.avatarLarge //profileImageURL
                                 placeholder:nil
                                     options:kNilOptions
                                     manager:[XWOpinionHelper avatarImageManager] ///< 圆角头像manager，内置圆角处理
                                    progress:nil
                                   transform:nil
                                  completion:nil];
...
//    NSLog(@"_profileView.top==%f", _profileView.top);
    ...
    NSURL *picBg = [XWOpinionHelper defaultURLForImageURL:layout.status.picBg];
        ...
        if (i >= picsCount) {
            ...
            switch (pic.largest.badgeType) {
                ...
                case WBPictureBadgeTypeLong: {
                    badge.layer.contents = (__bridge id)([XWOpinionHelper imageNamed:@"timeline_image_longimage"].CGImage);
                    badge.hidden = NO;
                } break;
                case WBPictureBadgeTypeGIF: {
                    badge.layer.contents = (__bridge id)([XWOpinionHelper imageNamed:@"timeline_image_gif"].CGImage);
                    badge.hidden = NO;
                } break;
            }
...
@implementation XWOpinionCell

- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    _statusView = [XWOpinionView new];
