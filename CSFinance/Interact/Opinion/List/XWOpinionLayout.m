...
@implementation XWOpinionLayout

- (instancetype)initWithStatus:(XWOpinion *)status style:(WBLayoutStyle)style {
...
- (void)_layoutTitle {
    ...
    XWOpinionTitle *title = _status.title;
    ...
    YYTextContainer *container = [YYTextContainer containerWithSize:CGSizeMake(kScreenW - 100, kWBCellTitleHeight)];
...
- (void)_layoutName {
    ...
    if (user.userVerifyType == WBUserVerifyTypeOrganization) {
        UIImage *blueVImage = [XWOpinionHelper imageNamed:@"avatar_enterprise_vip"];
    ...
    // VIP
    if (user.mbrank > 0) {
        UIImage *yelllowVImage = [XWOpinionHelper imageNamed:[NSString stringWithFormat:@"common_icon_membership_level%d",user.mbrank]];
        if (!yelllowVImage) {
            yelllowVImage = [XWOpinionHelper imageNamed:@"common_icon_membership"];
        }
...
- (void)_layoutSource {
    ...
    NSString *createTime = [XWOpinionHelper stringWithTimelineDate:_status.createdAt];
...
- (void)_layoutPicsWithStatus:(XWOpinion *)status isRetweet:(BOOL)isRetweet {
    if (isRetweet) {
...
- (void)_layoutCardWithStatus:(XWOpinion *)status isRetweet:(BOOL)isRetweet {
    if (isRetweet) {
        _retweetCardType = XWOpinionCardTypeNone;
        _retweetCardHeight = 0;
        _retweetCardTextLayout = nil;
        _retweetCardTextRect = CGRectZero;
    } else {
        _cardType = XWOpinionCardTypeNone;
        _cardHeight = 0;
        _cardTextLayout = nil;
    ...
    XWOpinionCardType cardType = XWOpinionCardTypeNone;
    ...
    if ((pageInfo.type == 11) && [pageInfo.objectType isEqualToString:@"video"]) {
        // 视频，一个大图片，上面播放按钮
        if (pageInfo.pagePic) {
            cardType = XWOpinionCardTypeVideo;
        ...
        if (cardTextLayout) {
            cardType = XWOpinionCardTypeNormal;
            cardHeight = 70;
        }
    }
...
- (void)_layoutTag {
    ...
    if (tag.tagType == 1) {
        _tagType = XWOpinionTagTypePlace;
        _tagHeight = 40;
        text.color = [UIColor colorWithWhite:0.217 alpha:1.000];
    } else {
        _tagType = XWOpinionTagTypeNormal;
        _tagHeight = 32;
        if (tag.urlTypePic) {
...
- (void)_layoutToolbar {
    ...
    NSMutableAttributedString *repostText = [[NSMutableAttributedString alloc] initWithString:_status.repostsCount <= 0 ? @"转发" : [XWOpinionHelper shortedNumberDesc:_status.repostsCount]];
    ...
    NSMutableAttributedString *commentText = [[NSMutableAttributedString alloc] initWithString:_status.commentsCount <= 0 ? @"评论" : [XWOpinionHelper shortedNumberDesc:_status.commentsCount]];
    ...
    NSMutableAttributedString *likeText = [[NSMutableAttributedString alloc] initWithString:_status.attitudesCount <= 0 ? @"赞" : [XWOpinionHelper shortedNumberDesc:_status.attitudesCount]];
    likeText.font = font;
...
- (NSMutableAttributedString *)_textWithStatus:(XWOpinion *)status
        ...
        do {
            ...
            if ([text attribute:YYTextHighlightAttributeName atIndex:range.location] == nil) {
                ...
                if (wburl.urlTypePic.length) {
                    // 链接头部有个图片附件 (要从网络获取)
                    NSURL *picURL = [XWOpinionHelper defaultURLForImageURL:wburl.urlTypePic];
    ...
    // 匹配 @用户名
    NSArray *atResults = [[XWOpinionHelper regexAt] matchesInString:text.string options:kNilOptions range:text.rangeOfAll];
    ...
    // 匹配 [表情]
    NSArray<NSTextCheckingResult *> *emoticonResults = [[XWOpinionHelper regexEmoticon] matchesInString:text.string options:kNilOptions range:text.rangeOfAll];
    ...
    for (NSTextCheckingResult *emo in emoticonResults) {
        ...
        NSString *imagePath = [XWOpinionHelper emoticonDic][emoString];
        UIImage *image = [XWOpinionHelper imageWithPath:imagePath];
        if (!image) continue;
...
- (NSAttributedString *)_attachmentWithFontSize:(CGFloat)fontSize imageURL:(NSString *)imageURL shrink:(BOOL)shrink {
    ...
    attachment.imageURL = [XWOpinionHelper defaultURLForImageURL:imageURL];
