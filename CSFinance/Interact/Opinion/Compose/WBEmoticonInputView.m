//
//  XWOpinionComposeEmoticonView.m
//  YYKitExample
//
...
- (void)updateContent {
    ...
    if (_isDelete) {
        _imageView.image = [XWOpinionHelper imageNamed:@"compose_emotion_delete"];
    } else if (_emoticon) {
        if (_emoticon.type == WBEmoticonTypeEmoji) {
            ...
            NSString *pngPath = [[XWOpinionHelper emoticonBundle] pathForScaledResource:_emoticon.png ofType:nil inDirectory:_emoticon.group.groupID];
            if (!pngPath) {
                NSString *addBundlePath = [[XWOpinionHelper emoticonBundle].bundlePath stringByAppendingPathComponent:@"additional"];
...
- (instancetype)initWithFrame:(CGRect)frame collectionViewLayout:(UICollectionViewLayout *)layout {
    ...
    _magnifier = [[UIImageView alloc] initWithImage:[XWOpinionHelper imageNamed:@"emoticon_keyboard_magnifier"]];
...
- (instancetype)init {
    self = [super init];
    self.frame = CGRectMake(0, 0, kScreenW, kViewHeight);
...
- (void)_initGroups {
    _emoticonGroups = [XWOpinionHelper emoticonGroups];
...
- (void)_initCollectionView {
    ...
    _collectionView = [[WBEmoticonScrollView alloc] initWithFrame:CGRectMake(0, 0, kScreenW, kOneEmoticonHeight * 3) collectionViewLayout:layout];
    ...
    _pageControl.size = CGSizeMake(kScreenW, 20);
...
- (void)_initToolbar {
    ...
    toolbar.size = CGSizeMake(kScreenW, kToolbarHeight);
    
    UIImageView *bg = [[UIImageView alloc] initWithImage:[XWOpinionHelper imageNamed:@"compose_emotion_table_right_normal"]];
    ...
    for (NSUInteger i = 0; i < _emoticonGroups.count; i++) {
        ...
        btn.left = kScreenW / (float)_emoticonGroups.count * i;
        btn.tag = i;
...
- (UIButton *)_createToolbarButton {
    ...
    btn.size = CGSizeMake(kScreenW / _emoticonGroups.count, kToolbarHeight);
    btn.titleLabel.font = [UIFont systemFontOfSize:14];
    [btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [btn setTitleColor:UIColorHex(5D5C5A) forState:UIControlStateSelected];
    
    UIImage *img;
    img = [XWOpinionHelper imageNamed:@"compose_emotion_table_left_normal"];
    ...
    img = [XWOpinionHelper imageNamed:@"compose_emotion_table_left_selected"];
    img = [img resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, img.size.width - 1) resizingMode:UIImageResizingModeStretch];
