//
//  XWOpinionComposeViewController.m
//  YYKitExample
//
...
#import "XWOpinionComposeTextParser.h"
#import "XWOpinionHelper.h"
#import "XWOpinionLayout.h"
#import "YYKit.h"

#define kToolbarHeight (35 + 46)

@interface XWOpinionComposeViewController() <YYTextViewDelegate, YYTextKeyboardObserver, XWOpinionComposeEmoticonViewDelegate>
...
@implementation XWOpinionComposeViewController
...
- (void)_initNavBar {
    ...
    switch (_type) {
        case XWOpinionComposeViewTypeStatus: {
            self.title = @"发观点";
        } break;
        case XWOpinionComposeViewTypeRetweet: {
            self.title = @"转发观点";
        } break;
        case XWOpinionComposeViewTypeComment: {
            self.title = @"发评论";
        } break;
    }
}
...
//    _textView.contentInset = UIEdgeInsetsMake(64, 0, kToolbarHeight, 0);
    ...
    _textView.textParser = [XWOpinionComposeTextParser new];
...
- (void)_initToolbar {
    ...
    [_toolbarPOIButton setImage:[XWOpinionHelper imageNamed:@"compose_locatebutton_ready"] forState:UIControlStateNormal];
    ...
    [_toolbarGroupButton setImage:[XWOpinionHelper imageNamed:@"compose_publicbutton"] forState:UIControlStateNormal];
...
- (UIButton *)_toolbarButtonWithImage:(NSString *)imageName highlight:(NSString *)highlightImageName {
    ...
    [button setImage:[XWOpinionHelper imageNamed:imageName] forState:UIControlStateNormal];
    [button setImage:[XWOpinionHelper imageNamed:highlightImageName] forState:UIControlStateHighlighted];
    button.centerY = 46 / 2;
...
- (void)_buttonClicked:(UIButton *)button {
    if (button == _toolbarPictureButton) {
        ...
        if (_textView.inputView) {
            ...
            [_toolbarEmoticonButton setImage:[XWOpinionHelper imageNamed:@"compose_emoticonbutton_background"] forState:UIControlStateNormal];
            [_toolbarEmoticonButton setImage:[XWOpinionHelper imageNamed:@"compose_emoticonbutton_background_highlighted"] forState:UIControlStateHighlighted];
        } else {
            ...
            [_textView reloadInputViews];
            [_textView becomeFirstResponder];
            [_toolbarEmoticonButton setImage:[XWOpinionHelper imageNamed:@"compose_keyboardbutton_background"] forState:UIControlStateNormal];
            [_toolbarEmoticonButton setImage:[XWOpinionHelper imageNamed:@"compose_keyboardbutton_background_highlighted"] forState:UIControlStateHighlighted];
        }
...
#pragma mark @protocol XWOpinionComposeEmoticonView
